...
就職活動を進める中で、選考情報の管理に Notion（Notion AI で構築したもの）を使っていましたが、使い込むほどに限界を感じるようになりました。

- 企業ごとに管理したい項目（選考状況、ES、面接記録、企業研究、予定・締切など）が多岐にわたり、Notion のデータベース機能だけでは横に並べるしかなく、情報が煩雑になっていく
- 見たい情報にたどり着くまでのページ遷移が多く、選考が10社、20社と増えるにつれて「あの企業、今どうなってたっけ」を確認するだけでもひと苦労だった
- 選考状況やインターンの日程が企業ごとにバラバラに存在し、進行度合いで並び替えることもできなかった
- 実際に、複数のインターンの開催期間が重なっていることに気づかず、直前でどちらかをキャンセルせざるを得なくなったこともあった

「就活管理に特化したツールなら、これらを一元化できるはずだ」という考えから、自分自身が実際に使うツールとして Career Compass の開発を始めました。

選考フロー・予定・ES・面接記録・企業研究・OB/OG 訪問といった情報を、企業ごとに整理された形で管理できるようにし、さらに振り返りデータを蓄積することで「自分がどの選考段階でつまずきやすいか」を可視化する分析機能まで踏み込んでいます。就活を「記録する」だけでなく、「改善につなげる」ツールを目指しています。

個人開発ながら、実際に友人数名にもデータを預けて使ってもらうサービスとして、セキュリティ（他ユーザーのデータ分離、削除処理の安全性）やデータ整合性（スキーマ変更時の後方互換性、保存失敗時のロールバック）を意識した設計を心がけました。
