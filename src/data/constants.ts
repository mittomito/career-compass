...
/**
 * 自由入力欄の最大文字数。
 * Firestore はドキュメントあたり 1MiB の上限があり、超えるとその企業への保存が
 * 以後すべて失敗するため、極端な長文が入らないよう入力側で制限する。
 */
export const MAX_LEN = {
  /** 企業名・インターン名・場所・期間名・ログインIDなどの短い項目 */
  short: 100,
  /** ステップ名・敗因タグ・カテゴリ名・予定の種類などのラベル */
  label: 50,
  /** URL */
  url: 500,
  /** 質問文（ES設問・面接で聞かれた質問・想定質問） */
  question: 500,
  /** 一言メモ（次回への一言・予定のメモ） */
  note: 200,
  /** 長文メモ（各種メモ・振り返り・企業研究） */
  memo: 3000,
  /** 最も長い本文（ES回答・想定回答・面接での自分の回答） */
  answer: 5000,
} as const
